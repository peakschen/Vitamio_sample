...
public class VideoView extends SurfaceView implements MediaController.MediaPlayerControl {
	...
	OnVideoSizeChangedListener mSizeChangedListener = new OnVideoSizeChangedListener() {
      ...
      Logs.d("onVideoSizeChanged: (%dx%d)", width, height);
  ...
  private OnErrorListener mErrorListener = new OnErrorListener() {
    public boolean onError(MediaPlayer mp, int framework_err, int impl_err) {
      Logs.d("Error: %d, %d", framework_err, impl_err);
  ...
  private OnInfoListener mInfoListener = new OnInfoListener() {
    ...
    public boolean onInfo(MediaPlayer mp, int what, int extra) {
      Logs.d("onInfo: (%d, %d)", what, extra);
    	
      	if(MediaPlayer.MEDIA_INFO_UNKNOW_TYPE == what){
    		Logs.e(" VITAMIO--TYPE_CHECK  stype  not include  onInfo mediaplayer unknow type ");
    	} 
  ...
  private OnTimedTextListener mTimedTextListener = new OnTimedTextListener() {
    ...
    public void onTimedTextUpdate(byte[] pixels, int width, int height) {
      Logs.i("onSubtitleUpdate: bitmap subtitle, %dx%d", width, height);
    ...
    public void onTimedText(String text) {
      Logs.i("onSubtitleUpdate: %s", text);
      if (mOnTimedTextListener != null)
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    Logs.d("VIDEO: %dx%dx%f, Surface: %dx%d, LP: %dx%d, Window: %dx%dx%f", mVideoWidth, mVideoHeight, mVideoAspectRatio, mSurfaceWidth, mSurfaceHeight, lp.width, lp.height, windowWidth, windowHeight, windowRatio);
    mVideoLayout = layout;
...
//      return;
    ...
    mContext.sendBroadcast(i);
		...
		//	options.put("cookies", "cookies");
			...
			Logs.e("Unable to open content: " + mUri, ex);
			mCurrentState = STATE_ERROR;
			mTargetState = STATE_ERROR;
  ...
  public void setHideMediaController(){
  ...
  public void setAdjustButton(int resourceId){
	  if(mMediaController!=null){
	  mMediaController.setAdJustButton(resourceId);
	  }
  }

  public void start() {
    Logs.i("cgf","----VideoView--start---"+mMediaController.getHasDismiss());
    if(mMediaController != null && mMediaController.getHasDismiss()){
      return;
    }
  ...
  public void endJumpToStart(){
    new Handler().post(new Runnable() {
      ...
      public void run() {
        mMediaPlayer.start();
        mMediaController.setSilentVolume();
        try {
          Thread.sleep(150);
        ...
        mMediaPlayer.pause();
        mMediaController.setCurrentVolume();
        mMediaController.setHasPlayEnd(true);
      }
    });
  }
  public void resumeKeepNormal(){
    new Handler().post(new Runnable() {
      @Override
      public void run() {
        mMediaController.setHasPlayEnd(false);
        mMediaPlayer.start();
        mMediaController.setSilentVolume();
        try {
          Thread.sleep(120);
  ...
  public void suspend() {
    if (isInPlaybackState()) {
      ...
      Logs.d("Unable to suspend video. Release MediaPlayer.");
    }
  }

  public void resume() {
    android.util.Log.i("cgf","----VideoView--resume---");
    if (mSurfaceHolder == null && mCurrentState == STATE_SUSPEND) {
      android.util.Log.i("cgf","----VideoView--resume1---");
      mTargetState = STATE_RESUME;
    } else if (mCurrentState == STATE_SUSPEND_UNSUPPORTED) {
      android.util.Log.i("cgf","----VideoView--resume2---");
      openVideo();
    }
  }
