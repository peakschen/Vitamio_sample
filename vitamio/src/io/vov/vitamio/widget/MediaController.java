...
import io.vov.vitamio.utils.Logs;
...
public class MediaController extends FrameLayout {
  ...
  private ImageButton mPauseButton,mediacontroller_zoom,mScreenToggle;
  private AudioManager mAM;
  private OnShownListener mShownListener;
  private OnHiddenListener mHiddenListener;
  /**已经播放到最后移到开头标识*/
  private boolean hasPlayEnd = false;
  /**播放暂停标识（针对onresume）*/
  private boolean hasPause = false;
  /**针对切换到其他界面应该停止播放*/
  private boolean hasDismiss = false;

  public void setHasDismiss(boolean hasDismiss) {
    this.hasDismiss = hasDismiss;
  }
  public boolean getHasDismiss(){
    return hasDismiss;
  }
  ...
  public void setHasPlayEnd(boolean hasPlayEnd) {
    this.hasPlayEnd = hasPlayEnd;
  }
  public boolean getHasPlayEnd(){
    return hasPlayEnd;
  }
  ...
  private OnClickListener onClickListener = new OnClickListener() {
    public void onClick(View v) {
    	if(v.getId()==getResources().getIdentifier("mediacontroller_play_pause", "id", mContext.getPackageName())){
				doPauseResume();
				show(sDefaultTimeout);
    	}else if(v.getId()==getResources().getIdentifier("mediacontroller_zoom", "id", mContext.getPackageName())){
				hide();
				try {
					if (((Activity) mContext).getRequestedOrientation() == ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE) {
                      //build/intermediates/exploded-aar/com.android.support/appcompat-v7/23.3.0/res/
						mediacontroller_zoom.setImageResource(getResources().getIdentifier("zoom", "drawable",mContext.getPackageName()));
						((Activity) mContext).setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
					} else {
						mediacontroller_zoom.setImageResource(getResources().getIdentifier("narrow", "drawable",mContext.getPackageName()));
    	...
    	}else if(v.getId()==getResources().getIdentifier("mediacontroller_adjust", "id", mContext.getPackageName())){
//    		  EventBus.getDefault().post(new ZoomEvent(3));
    	}
  ...
  private void initControllerView(View v) {
    ...
    mediacontroller_zoom = (ImageButton) v.findViewById(getResources().getIdentifier("mediacontroller_zoom", "id", mContext.getPackageName()));
    mScreenToggle = (ImageButton) v.findViewById(getResources().getIdentifier("mediacontroller_adjust", "id", mContext.getPackageName()));
    
    if (mPauseButton != null) {
      mPauseButton.requestFocus();
      mPauseButton.setOnClickListener(onClickListener);
      mediacontroller_zoom.setOnClickListener(onClickListener);
      mScreenToggle.setOnClickListener(onClickListener);
    }
  ...
  public void show(int timeout) {
    if (!mShowing && mAnchor != null && mAnchor.getWindowToken() != null) {
      ...
      if (mFromXml) {
        ...
        //setWindowLayoutType();
//        EventBus.getDefault().post(new ZoomEvent(1));
        mWindow.showAsDropDown(mAnchor,0,-dip2px(mContext, 40));
      }
      mShowing = true;
    ...
    if (mShowing) {
      try {
        ...
        if (mFromXml)
          ...
          mWindow.dismiss();
//          EventBus.getDefault().post(new ZoomEvent(2));
        }
      } catch (IllegalArgumentException ex) {
        Logs.d("MediaController already removed");
      }
      mShowing = false;
  ...
  public void setAdJustButton(int resid){
	   mScreenToggle.setBackgroundResource(resid);
  }
  ...
  public void setSilentVolume(){
    ...
    Log.i("cgf","----setSilentVolume---"+volume);
  }
